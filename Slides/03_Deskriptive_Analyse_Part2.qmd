...
bestimee_kundenbindung(7000, 3) # Sollte "Gold" ausgeben
...
def bestimme_kundenbindung(umsatz, transaktionen):
    ...
    if umsatz <= 1000 and transaktionen < 10:
        kategorie = "Bronze"
    elif (umsatz > 1000 and umsatz <= 5000) or (transaktionen >= 10 and transaktionen < 30):
        kategorie = "Silber"
    elif (umsatz > 5000 and umsatz <= 10000) or (transaktionen >= 30 and transaktionen < 60):
        kategorie = "Gold"
    else:
        kategorie = "Platin"
...
q1 = df["kostenüberschreitung_abs"].quantile(0.01)
q99 = df["kostenüberschreitung_abs"].quantile(0.99)
df = df.query("kostenüberschreitung_abs > @q1 and kostenüberschreitung_abs < @q99")

# Kostenüberschreitung relativ
q1 = df["kostenüberschreitung_rel"].quantile(0.01)
q99 = df["kostenüberschreitung_rel"].quantile(0.99)
df = df.query("kostenüberschreitung_rel > @q1 and kostenüberschreitung_rel < @q99")

# Verzögerung absolut
q1 = df["verzögerung_abs"].quantile(0.01)
q99 = df["verzögerung_abs"].quantile(0.99)
df = df.query("verzögerung_abs > @q1 and verzögerung_abs < @q99")

# Verzögerung relativ
q1 = df["verzögerung_rel"].quantile(0.01)
q99 = df["verzögerung_rel"].quantile(0.99)
df = df.query("verzögerung_rel > @q1 and verzögerung_rel < @q99")
```
...
__Funktion anwenden:__ Wir wenden die Funktion auf die vier Spalten an.

```{.python}
df = remove_outliers(df, "kostenüberschreitung_abs")
df = remove_outliers(df, "kostenüberschreitung_rel")
df = remove_outliers(df, "verzögerung_abs")
df = remove_outliers(df, "verzögerung_rel")
```
